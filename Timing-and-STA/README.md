...
| **Project** | RTL Mini-Projects — 01 |
| **Target HDL** | Verilog (IEEE 1364-2001 Synthesizable Subset) |
...
Design and verify a synchronous, parameterizable Moore FSM in synthesizable Verilog. The design integrates an internal cycle-accurate timer register with registered state logic, guaranteeing latch-free synthesis and glitch-free output transitions.
...
**Out of Scope**
* Pedestrian walk requests and emergency vehicle preemption (requires CDC synchronization).
* Multi-axis cross-traffic arbitration.
* Runtime dynamic timing configuration via a software-accessible bus (e.g., APB/AXI).

## 4. Operational Assumptions

* Single clock domain driving all flip-flop clock pins via a balanced clock tree.
* Synchronous active-high reset, sampled deterministically on `posedge clk`.
* Output bus `light[2:0]` drives downstream registered logic or pad drivers within standard fanout limits.

## 5. Functional Requirements

| ID | Requirement Description | Verification Method |
|---|---|---|
| **REQ-F01** | Cycle through `S_RED` -> `S_GREEN` -> `S_YELLOW` -> `S_RED` in strict sequence without skipping states. | Automated self-checking sequence task |
| **REQ-F02** | Each state holds its active output for exactly N clock cycles defined by parameters (`RED_CYCLES`, `GREEN_CYCLES`, `YELLOW_CYCLES`). | Automated cycle-accurate counter monitor |
| **REQ-F03** | Assertion of synchronous `reset` forces the FSM to `S_RED` and clears the timer within 1 clock cycle. | Synchronous reset recovery check |
| **REQ-F04** | Output bus `light[2:0]` must be one-hot at all times; multiple active lights is an invalid condition. | Cycle-by-cycle one-hot check (`$onehot` monitor) |
| **REQ-F05** | The core runs continuously over extended run times without state lockup or illegal states. | 50+ cycle randomized stress simulation |

## 6. Non-Functional & Microarchitecture Constraints

| ID | Requirement Description | Verification Method |
|---|---|---|
| **REQ-NF01** | Fully synthesizable RTL using standard Verilog constructs; zero behavioral delay `#` in datapath. | Elaboration & Synthesis Check (Vivado / Yosys) |
| **REQ-NF02** | Latch-free design: complete branch assignments in all combinational blocks. | Verilator Static Lint (`-Wall --lint-only`) |
| **REQ-NF03** | Synchronous reset architecture eliminating de-assertion recovery/removal race conditions. | Static Timing & Reset Path Review |
| **REQ-NF04** | All state durations parameterized via `parameter` declarations, avoiding hardcoded magic numbers. | Compile-Time Parameter Override Test |
...
The controller partitions timing from control logic across two functional processes:
   ...
   clk, reset  |     Sequential Process            Combinational Process|
        ...
        v      |              v                              v          |
   +---------+ |       +-------------+   current   +-------------------+|   light[2:0]
   |  Timer  | +-----> | State Reg   | --state---> |  Output Decode    |+------------>
   | Counter | |       +------+------+             +-------------------+|
   +----+----+ |              |                              ^          |
        ^      |              +-------- Next State ----------+          |
        +------+------------------ State Feedback ----------------------+
...
| Requirement ID | Verification Test Case | Expected Result | Status |
|---|---|---|---|
| **REQ-F01** | `TC_03` (Sequential Transition Integrity) | Order verified: `RED` -> `GREEN` -> `YELLOW` | [PASS] |
| **REQ-F02** | `TC_02` (State Timing Hold Verification) | Cycle counts match parameter values | [PASS] |
| **REQ-F03** | `TC_01` (Reset Latency Check) | Immediate return to `S_RED` on clock edge | [PASS] |
| **REQ-F04** | `TC_03` (Output Mutual Exclusion) | Exactly one light active every cycle | [PASS] |
| **REQ-F05** | `TC_04` (Extended Run Stress) | Zero deadlock or undefined states over 50 cycles | [PASS] |
| **REQ-NF02** | Verilator Lint Check | 0 Inferred Latches, clean elaboration | [PASS] |
